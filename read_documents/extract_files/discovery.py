...
def normalize_path(path: str) -> str:
    ...
    在 Windows 的 Git Bash（MSYS2）下，Shell 将 ``/tmp/`` 视为 WindowsTEMP 目录，
...
def find_files(root: str, pattern: str | None = None, max_depth: int = 6) -> list[str]:
    ...
    Args:
        root (str): 搜索的根目录。
        pattern (str | None): 可选的子串模式（不区分大小写），
            仅匹配文件名或路径中包含该子串的文件。
            
        max_depth (int): 最大目录深度，超过则不再进入子目录（默认 6）。
    
    Returns:
        path_list (list[str]): 找到的文件路径列表。
    """
    results: list[str] = []
