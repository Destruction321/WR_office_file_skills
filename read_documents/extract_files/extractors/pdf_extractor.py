...
def extract_pdf(filepath: Path, assets_dir: Path | None = None) -> list[str]:
    ...
    if assets_dir:
        assets_result = assets.extract_pdf_assets(
            filepath, assets_dir / filepath.stem)
    ...
    if pdfplumber_open:
        try:
            with safe_open_path(filepath) as safe_path:
                with pdfplumber_open(safe_path) as pdf:  # type: ignore[operator]
    ...
    try:
        with safe_open_path(filepath) as safe_path:
            reader = PdfReader(safe_path)  # type: ignore[operator]
